...
class blowfish(object):
    # Blowfish_initstate
    def __init__(self, s_boxes=blowfish_s_boxes, p_subkey=blowfish_p_subkey):
        self.s_boxes = [list(n) for n in s_boxes]
        self.p_subkey = [n for n in p_subkey]

    def expandstate(self, data, databytes, key, keybytes):
        BLF_N = 16 # Todo: Make constant?
        datal = 0
        datar = 0
        self.current = 0
    ...
    def expand0state(self, key, keybytes):
        BLF_N = 16 # Todo: Make constant?
        datal = 0
        datar = 0
        self.current = 0
    ...
    def encipher(self, xl, xr):
        ...
        for n in range(1,17):
            if n%2:
                xr ^= (self.F(s,xl) & 0xffffffff) ^ p[n]
            else:
                xl ^= (self.F(s,xr) & 0xffffffff) ^ p[n] 
        # This returns "(xl, xr)", would be wasteful to do the name swapping
        return xr^p[17], xl